...
exports.compare = function compare() {
    ...
    const oldData = JSON.parse(fs.readFileSync('./JSON/OldData.json', 'utf8'));

    // Read the NewData JSON file b
    const newData = JSON.parse(fs.readFileSync('./JSON/NewData.json', 'utf8'));
    ...
    for (const newObject of newData) {
        ...
        if (oldObject) {
            // The object exists in both files. Check if the size has changed. Size has changed.
            ...
            // The object does not exist in the OldData file. New trade has been opened.
    ...
    for (const oldObject of oldData) {
        ...
        if (!newObject) {
            // The object does not exist in the NewData file/ Trade has been closed. 
