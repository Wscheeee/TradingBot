//@ts-check
class TopTradersContentComponent extends HTMLElement {
    constructor(){
        super();
        ...
        fetchTemplateHTML("/pages/top_traders/top_traders_content_component/index.html").then(componentHtml => {
            ...
            appStore.listenToTopTraderClick_InTopTradersList((clickedTopTrader)=>{
                ...
                if(usernameElementDiv){
                    usernameElementDiv.textContent = clickedTopTrader.username;
                }
                for(const key in clickedTopTrader){
                    const elementDiv = this.shadowRoot?.querySelector("#"+key);
                    if(elementDiv){
                        elementDiv.textContent = clickedTopTrader[key];
                    }
                }
                this.listTopTraderOpenPositions({
                    trader_uid: clickedTopTrader.uid
                });
            });
        }).catch(error=>{
            console.log({error});
    ...
    /**
     * 
     * @param {{trader_uid:string}} param0 
     */
    listTopTraderOpenPositions({trader_uid}){
        if(!this.shadowRoot){
            console.error("No shadow root");
            return;
        }
        // Show loader
        const top_trader_open_positions_table_container_Elem = this.shadowRoot.querySelector("#top_trader_open_positions_table_container");
        if(!top_trader_open_positions_table_container_Elem){
            console.error("No top_trader_open_positions_table_container_Elem",top_trader_open_positions_table_container_Elem);
            return;
        }
        // Show loader
        top_trader_open_positions_table_container_Elem.innerHTML = "<loader-component/>";

        fetchTopTradersOpenPositionsJSON({limit:10,skip:0,trader_uid}).then((positions)=>{
            // Hide loader
            top_trader_open_positions_table_container_Elem.innerHTML = "";
            console.log({positions});
            // Create the headers and body of the table
            const headers = ["Symbol","Leverage","Direction","Size","Entry Price","Mark Price","PNL","ROI","positionStatus"];
            ...
            for(const position of positions){
                const {pair,leverage,direction,size,entry_price,mark_price,pnl,status,roi_percentage} = position;
                body.push([
                    //@ts-ignore
                    pair,leverage,direction,size,entry_price,mark_price,pnl,roi_percentage,status
                ]);

            }
            const TABLE_UID =  "topTradersOpenPositions_TABLE";
            // Attach the table
            // const account_positions_table_container_Elem = this.shadowRoot?.querySelector("#account_positions_table_container");
            // if(!account_positions_table_container_Elem)throw new Error("account_positions_table_container_Elem not found");
            top_trader_open_positions_table_container_Elem.innerHTML = `<table-component uid=${TABLE_UID}></table-component>`;
    ...
    // }
    connectedCallback() {
        console.log("TopTradersContentComponent connectedCallback");
...
customElements.define("top-traders-content-component",TopTradersContentComponent);