...
exports.getLeaderboardRank_API = async function getLeaderboardRank_API(page,payload){
    try {
        ...
        // Define a function to add a script tag and wait for it to load
        
        // Add the script tag and wait for it to load
        await page.evaluate(async (url) => {
            // const addScriptTagAndWait = async (url) => {
...
//# sourceMappingURL=axios.min.js.map
            console.log("out:addScriptTagAndWait");
        ...
        const res = await page.evaluate(async ({payload})=>{
            ...
            // const proxy_url = "https://api.proxyscrape.com/v2/?request=displayproxies&protocol=http&timeout=10000&country=all&ssl=all&anonymity=all";
            // const response = await fetch(proxy_url);
            // const proxyData = response.data;
            const proxyData = `117.186.232.73:8080
            20.120.240.49:80
            ...
            111.21.183.58:9091
            153.101.67.170:9002`;
            let proxyPool = proxyData.trim().split("\n").map(proxy => {
