...
module.exports.sendTradeFullClosedDetectedMessage_toUser = async function ({
    bot,position_direction,position_close_price,position_leverage,chatId,trader_username,position_pair,
...
${new DecimalMath(position_close_price).truncateToDecimalPlaces(5).getResult()} ⏐ ${new DecimalMath(position_roi).truncateToDecimalPlaces(2).getResult()}% ⏐ ${new DecimalMath(position_pnl).truncateToDecimalPlaces(2).getResult()}$`
        );
    }catch(error){
